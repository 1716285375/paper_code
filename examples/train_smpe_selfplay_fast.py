...
# -*- coding: utf-8 -*-
"""
@File           : train_smpe_selfplay_fast.py
@Author         : Jie
@CopyRight      : Copyright © 2025 Jie. All Rights Reserved
@Create Date    : 2025-11-04 00:00
@Update Date    :
@Description    : SMPE²自博弈训练脚本（快速验证版本）
针对快速原型验证和8GB显存优化的SMPE自博弈训练脚本
...
from algorithms.smpe import SMPESelfPlayTrainer, SMPEPolicyAgent
...
def main():
    parser = argparse.ArgumentParser(description="SMPE²自博弈训练（快速验证版本）")
    parser.add_argument(
        "--config", 
        type=str, 
        default="configs/smpe/smpe_magent2_selfplay_12v12_fast_verify.yaml",
        help="配置文件路径"
    )
    parser.add_argument("--resume", type=str, default=None, help="恢复训练的检查点路径")
    parser.add_argument("--map_size", type=int, default=None, help="覆盖配置中的map_size")
    parser.add_argument("--batch_size", type=int, default=None, help="覆盖配置中的batch_size")
    ...
    if args.map_size is not None:
        config["env"]["kwargs"]["map_size"] = args.map_size
    ...
    # 获取state维度（从环境获取，如果环境提供）
    try:
        state_dim = env.get_state_size()
        print(f"✓ 从环境获取state维度: {state_dim}")
    except:
        state_dim = config.get("agent", {}).get("state_dim", obs_dim * len(agent_ids))
        print(f"⚠ 无法从环境获取state维度，使用配置值: {state_dim}")

    print(f"\n{'='*60}")
    print(f"环境信息")
    print(f"{'='*60}")
    print(f"总Agent数: {len(agent_ids)}")
    print(f"红队Agent: {red_agents[:5]}... ({len(red_agents)}个)" if len(red_agents) > 5 else f"红队Agent: {red_agents} ({len(red_agents)}个)")
    print(f"蓝队Agent: {blue_agents[:5]}... ({len(blue_agents)}个)" if len(blue_agents) > 5 else f"蓝队Agent: {blue_agents} ({len(blue_agents)}个)")
    ...
    n_agents_total = len(all_agent_ids)
    ...
    config_name = config_path.stem  # 获取文件名（不含扩展名）
    # 例如：smpe_magent2_selfplay_12v12_8h.yaml -> smpe_magent2_selfplay_12v12_8h
    checkpoint_dir = config.get("checkpoint_dir", None)
    if checkpoint_dir is None:
        # 自动生成：checkpoints/{config_name}
        checkpoint_dir = f"checkpoints/{config_name}"
    
    # 将checkpoint_dir添加到training_config中，供trainer使用
    training_config["checkpoint_dir"] = checkpoint_dir
    
    # 创建checkpoint目录
    checkpoint_path_obj = Path(checkpoint_dir)
    checkpoint_path_obj.mkdir(parents=True, exist_ok=True)
    print(f"✓ Checkpoint目录: {checkpoint_dir}")

    # 日志记录器
    logger = LoggerManager(
        name="smpe_selfplay_fast_training",
    ...
    if tracking_config.get("enabled", True):
        ...
        if tracking_config.get("wandb", {}).get("enabled", False):
            ...
            project = wandb_config.get("project", "magent2-smpe-selfplay-fast")
            ...
            if name is None:
                ...
                name = f"{env_name}_fast_{timestamp}"
            wandb_tracker.init(
                project=project,
                name=name,
                config=config,
        ...
        if tracking_config.get("tensorboard", {}).get("enabled", False):
            ...
            project = tb_config.get("project", "magent2-smpe-selfplay-fast")
            name = tb_config.get("name", None) or "smpe_selfplay_fast"
            tb_tracker.init(
    ...
    if data_saving_config.get("enabled", False):
        ...
        if data_output_dir is None:
            # 自动生成：training_data/{config_name}
            data_output_dir = f"training_data/{config_name}"
        
        data_manager = TrainingDataManager(
            output_dir=data_output_dir,
            save_format=data_saving_config.get("format", "json"),  # "json", "csv", "both"
        )
    ...
    # 开始训练
    num_updates = training_config.get("num_updates", 500)
    print(f"\n{'='*60}")
    print(f"开始SMPE²自博弈训练（快速验证版本）")
    print(f"{'='*60}")
    print(f"更新次数: {num_updates}")
    print(f"Batch size: {training_config.get('batch_size', 64)}")
    print(f"Episode长度: {training_config.get('max_steps_per_episode', 30)}")
    print(f"设备: {device}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        checkpoint_path = project_root / checkpoint_dir / "interrupted_smpe_checkpoint.pt"
        ...
        if data_manager is not None:
            try:
                saved_paths = data_manager.save_all(prefix="smpe_selfplay_fast_training")
                print("\n✅ Training data saved:")
        ...
        if tracker and tracker.is_initialized:
            try:
                ...
                print(f"⚠ Failed to finish tracker: {e}")
