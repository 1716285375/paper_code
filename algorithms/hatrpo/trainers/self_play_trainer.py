...
@Update Date    :
@Description    : HATRPO自博弈训练器
实现HATRPO算法的自博弈训练，适用于对抗性多Agent环境
"""
# ------------------------------------------------------------
...
class SelfPlayHATRPOTrainer(HATRPOTrainer):
    """
    HATRPO自博弈训练器
    
    继承自HATRPOTrainer，添加自博弈功能
    """
    ...
    def __init__(
        ...
        if not isinstance(agent, AgentManager):
            raise ValueError("SelfPlayHATRPOTrainer requires AgentManager")
    ...
    def train(self, num_updates: int) -> None:
        ...
        if self.logger:
            log_msg = f"开始HATRPO自博弈训练，共 {num_updates} 个更新..."
            ...
            print(f"开始HATRPO自博弈训练，共 {num_updates} 个更新...")
        ...
        for update in range(num_updates):
            ...
            if self.update_count % self.save_freq == 0:
                ...
                self.save(f"{checkpoint_dir}/hatrpo_selfplay_checkpoint_{self.update_count}.pt")

    def _train_step_self_play(self, processed_data: Dict[str, Any]) -> Dict[str, float]:
        """
        自博弈训练步骤（使用HATRPO的顺序TRPO更新，只训练主团队）
        """
        # 只训练主团队的Agent
        main_agent_ids = self.agent.get_group_members(self.main_team)
        
        # 过滤只包含主团队的数据
        main_processed_data = {aid: processed_data[aid] for aid in main_agent_ids if aid in processed_data}
        
        # 使用HATRPO的顺序更新方法（已在HATRPOTrainer中实现）
        return self._train_step_multi_agent(main_processed_data)
