...
@Update Date    :
@Description    : MATRPO自博弈训练器
实现MATRPO算法的自博弈训练，适用于对抗性多Agent环境
"""
# ------------------------------------------------------------
...
class SelfPlayMATRPOTrainer(MATRPOTrainer):
    """
    MATRPO自博弈训练器
    
    继承自MATRPOTrainer，添加自博弈功能
    """
    ...
    def __init__(
        ...
        if not isinstance(agent, AgentManager):
            raise ValueError("SelfPlayMATRPOTrainer requires AgentManager")
    ...
    def train(self, num_updates: int) -> None:
        ...
        if self.logger:
            log_msg = f"开始MATRPO自博弈训练，共 {num_updates} 个更新..."
            ...
            print(f"开始MATRPO自博弈训练，共 {num_updates} 个更新...")
        ...
        for update in range(num_updates):
            ...
            if self.update_count % self.save_freq == 0:
                ...
                self.save(f"{checkpoint_dir}/matrpo_selfplay_checkpoint_{self.update_count}.pt")
    ...
    def _train_step_self_play(self, processed_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        # 使用MATRPO的TRPO更新方法（已在MATRPOTrainer中实现）
