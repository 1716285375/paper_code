...
@Update Date    :
@Description    : HAPPO自博弈训练器
实现HAPPO算法的自博弈训练，适用于对抗性多Agent环境
"""
# ------------------------------------------------------------
...
class SelfPlayHAPPOTrainer(HAPPOTrainer):
    """
    HAPPO自博弈训练器
    
    继承自HAPPOTrainer，添加自博弈功能
    """
    ...
    def __init__(
        ...
        if not isinstance(agent, AgentManager):
            raise ValueError("SelfPlayHAPPOTrainer requires AgentManager")
    ...
    def train(self, num_updates: int) -> None:
        ...
        if self.logger:
            log_msg = f"开始HAPPO自博弈训练，共 {num_updates} 个更新..."
            ...
            print(f"开始HAPPO自博弈训练，共 {num_updates} 个更新...")
        ...
        for update in range(num_updates):
            ...
            if self.update_count % self.save_freq == 0:
                ...
                self.save(f"{checkpoint_dir}/happo_selfplay_checkpoint_{self.update_count}.pt")

    def _train_step_self_play(self, processed_data: Dict[str, Any]) -> Dict[str, float]:
        """
        自博弈训练步骤（使用HAPPO的顺序更新，只训练主团队）
        """
        # 只训练主团队的Agent
